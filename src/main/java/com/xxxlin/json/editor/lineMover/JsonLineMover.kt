...
public final class JsonLineMover extends LineMover {
    ...
    public boolean checkAvailable(@NotNull Editor editor, @NotNull PsiFile file, @NotNull MoveInfo info, boolean down) {
        ...
        if (!(file instanceof com.xxxlin.json.psi.JsonFile) || !super.checkAvailable(editor, file, info, down)) {
            return false;
        }
        ...
        if (destElementRange != null) {
            ...
            if (destFirst == destSecond && !(destFirst instanceof JsonProperty) && !(destFirst instanceof JsonValue)) {
                ...
                if (((com.xxxlin.json.psi.JsonFile) parent.getContainingFile()).getTopLevelValue() == parent) {
                    info.prohibitMove();
                    return true;
                }
            }
    ...
    private static @NotNull Pair<PsiElement, PsiElement> expandCommentsInRange(@NotNull Pair<PsiElement, PsiElement> range) {
        final PsiElement upper = com.xxxlin.json.psi.JsonPsiUtil.findFurthestSiblingOfSameType(range.getFirst(), false);
