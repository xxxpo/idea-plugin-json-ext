...
import com.xxxlin.json.psi.impl.JsonCollectionPsiPresentationUtils;
...
public final class JsonFoldingBuilder implements FoldingBuilder, DumbAware {
    ...
    public @Nullable String getPlaceholderText(@NotNull ASTNode node) {
        ...
        if (type == JsonElementTypes.OBJECT) {
            ...
            if (candidate != null) {
                ...
                return "properties count=" + properties.size();
            }
        } else if (type == JsonElementTypes.ARRAY && node.getPsi() instanceof JsonArray arrayNode) {
            return "array size=" + arrayNode.getValueList().size();
        } else if (type == JsonElementTypes.LINE_COMMENT1) {
            return "//...";
    ...
    public static @NotNull Couple<PsiElement> expandLineCommentsRange(@NotNull PsiElement anchor) {
        return Couple.of(com.xxxlin.json.psi.JsonPsiUtil.findFurthestSiblingOfSameType(anchor, false), JsonPsiUtil.findFurthestSiblingOfSameType(anchor, true));
    }
