...
class JsonSmartKeysConfigurable : BeanConfigurable<Unit>(Unit), SearchableConfigurable {
    ...
    init {
        JsonEditorOptions.getInstance()?.let { settings ->
            ...
            checkBox(
                JsonBundle.message("settings.smart.keys.automatically.manage.commas.when.pasting.json.fragments"),
                settings::COMMA_ON_PASTE
            )
            checkBox(
                JsonBundle.message("settings.smart.keys.escape.text.on.paste.in.string.literals"),
                settings::ESCAPE_PASTED_TEXT
            )
            checkBox(
                JsonBundle.message("settings.smart.keys.automatically.add.quotes.to.property.names.when.typing.comma"),
                settings::AUTO_QUOTE_PROP_NAME
            )
            checkBox(
                JsonBundle.message("settings.smart.keys.automatically.add.whitespace.when.typing.comma.after.property.names"),
                settings::AUTO_WHITESPACE_AFTER_COLON
            )
            checkBox(
                JsonBundle.message("settings.smart.keys.automatically.move.comma.after.the.property.name.if.typed.inside.quotes"),
